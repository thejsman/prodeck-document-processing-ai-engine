...
function buildPath(position: 'top-left' | 'top-right' | 'bottom-left' | 'bottom-right'): string {
  switch (position) {
    case 'top-left': {
      const ox = 200, oy = 65;
      return [`M ${ox},${oy}`,`L ${ox + PS},${oy}`,`L ${ox + PS},${oy + HS - TR}`,`C ${ox + PS + TR},${oy + HS - TR} ${ox + PS + TR},${oy + HS + TR} ${ox + PS},${oy + HS + TR}`,`L ${ox + PS},${oy + PS}`,`L ${ox + HS + TR},${oy + PS}`,`C ${ox + HS + TR},${oy + PS + TR} ${ox + HS - TR},${oy + PS + TR} ${ox + HS - TR},${oy + PS}`,`L ${ox},${oy + PS}`,'Z'].join(' ');
    }
    case 'top-right': {
      const ox = 390, oy = 65;
      return [`M ${ox},${oy}`,`L ${ox + PS},${oy}`,`L ${ox + PS},${oy + PS}`,`L ${ox + HS + TR},${oy + PS}`,`C ${ox + HS + TR},${oy + PS + TR} ${ox + HS - TR},${oy + PS + TR} ${ox + HS - TR},${oy + PS}`,`L ${ox},${oy + PS}`,`L ${ox},${oy + HS + TR}`,`C ${ox - TR},${oy + HS + TR} ${ox - TR},${oy + HS - TR} ${ox},${oy + HS - TR}`,'Z'].join(' ');
    }
    case 'bottom-left': {
      const ox = 200, oy = 255;
      return [`M ${ox},${oy}`,`L ${ox + HS - TR},${oy}`,`C ${ox + HS - TR},${oy - TR} ${ox + HS + TR},${oy - TR} ${ox + HS + TR},${oy}`,`L ${ox + PS},${oy}`,`L ${ox + PS},${oy + HS - TR}`,`C ${ox + PS + TR},${oy + HS - TR} ${ox + PS + TR},${oy + HS + TR} ${ox + PS},${oy + HS + TR}`,`L ${ox + PS},${oy + PS}`,`L ${ox},${oy + PS}`,'Z'].join(' ');
    }
    case 'bottom-right': {
      const ox = 390, oy = 255;
      return [`M ${ox},${oy}`,`L ${ox + HS - TR},${oy}`,`C ${ox + HS - TR},${oy - TR} ${ox + HS + TR},${oy - TR} ${ox + HS + TR},${oy}`,`L ${ox + PS},${oy}`,`L ${ox + PS},${oy + PS}`,`L ${ox},${oy + PS}`,`L ${ox},${oy + HS + TR}`,`C ${ox - TR},${oy + HS + TR} ${ox - TR},${oy + HS - TR} ${ox},${oy + HS - TR}`,'Z'].join(' ');
    }
  }
}
...
const ICON_PATHS: Record<string, string> = {
  ...
  process:  'M 0,0 m -8,0 a 8,8 0 1,0 16,0 a 8,8 0 1,0 -16,0 M 0,-14 L 0,-11 M 0,11 L 0,14 M -14,0 L -11,0 M 11,0 L 14,0',
...
> = {
  'top-left':     { x: 185, y: 158, anchor: 'end'   },
  'top-right':    { x: 585, y: 158, anchor: 'start' },
  'bottom-left':  { x: 185, y: 348, anchor: 'end'   },
  'bottom-right': { x: 585, y: 348, anchor: 'start' },
};

function getPieceColor(position: 'top-left' | 'top-right' | 'bottom-left' | 'bottom-right', tokens: PluginTokens): string {
  return (position === 'top-left' || position === 'bottom-right') ? tokens.accent : (tokens.accentDim || `${tokens.accent}99`);
}
...
export function PuzzleDiagram({ data, tokens }: Props) {
  ...
  const bgGradId = `puzzle-bg-grad-${uid}`;
    ...
    >
      <defs>
        ...
        {pieces.map((piece) => {
          ...
          const { ox, oy } = origins;
          return (
            <clipPath key={`clip-${pos}-${uid}`} id={`clip-${pos}-${uid}`}>
      ...
      <rect x={0} y={0} width={800} height={520} fill={`url(#${bgGradId})`} />
      ...
      {pieces.map((piece, i) => {
        ...
        return (
          <g key={i}>
            <path d={buildPath(pos)} fill={fill} opacity={0.92} />
            <g transform={`translate(${iconCx},${iconCy})`}>
              <path d={iconPath} stroke="white" strokeWidth={2.2} fill="none" strokeLinecap="round" strokeLinejoin="round" />
            </g>
            {labelPos && (
              <text
