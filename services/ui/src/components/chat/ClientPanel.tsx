...
export function ClientPanel({
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      ...
      if (fileDropdownRef.current && !fileDropdownRef.current.contains(e.target as Node) && btn && !btn.contains(e.target as Node))
        setMenuFile(null);
    };
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      ...
      if (proposalDropdownRef.current && !proposalDropdownRef.current.contains(e.target as Node) && btn && !btn.contains(e.target as Node))
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      ...
      if (micrositeDropdownRef.current && !micrositeDropdownRef.current.contains(e.target as Node) && btn && !btn.contains(e.target as Node))
  ...
  useEffect(() => {
    ...
    fetchProposals(apiKey)
      .then(all => setProposals(namespace, all.filter(p => p.fileName.startsWith(`${namespace}::`))))
      .catch(() => {})
  ...
  useEffect(() => {
    const justCompleted = Object.values(allExecutions).filter(
      e => (e.type === 'proposal' || e.type === 'microsite') && e.status === 'completed' && !seenCompletedRef.current.has(e.id),
    );
    Object.values(allExecutions).filter(e => e.status === 'completed' || e.status === 'failed').forEach(e => seenCompletedRef.current.add(e.id));
    if (!justCompleted.length || !namespace || !apiKey) return;
    if (justCompleted.some(e => e.type === 'proposal')) {
      fetchProposals(apiKey).then(all => setProposals(namespace, all.filter(p => p.fileName.startsWith(`${namespace}::`))));
    }
  ...
  const renderArtifacts = () => (
    <div className="client-panel-list">
      ...
      <span className="client-panel-sub-label" style={{ marginTop: 8 }}>Microsites</span>
      {loadingMicrosites ? (
        <EmptyRow label="Loading…" />
          ...
          >
            ...
            <button
              ...
              style={{
                ...
                padding: '1px 5px', border: 'none', lineHeight: 1,
              ...
              onClick={e => {
                e.stopPropagation();
                if (isMenuOpen) { setMenuFile(null); return; }
  ...
  return (
    <>
      ...
      {menuProposal && createPortal(
        <div ref={proposalDropdownRef} className="card" style={{ position: 'fixed', top: proposalMenuPos.top, right: proposalMenuPos.right, minWidth: 120, padding: '4px 0', zIndex: 99999 }}>
          <button className="btn btn-sm" style={{ width: '100%', textAlign: 'left', borderRadius: 0, border: 'none', justifyContent: 'flex-start', padding: '8px 14px', fontSize: 14, color: 'var(--danger)', gap: 8 }}
            onMouseDown={e => e.preventDefault()}
      ...
      {menuMicrosite && createPortal(
        <div ref={micrositeDropdownRef} className="card" style={{ position: 'fixed', top: micrositeMenuPos.top, right: micrositeMenuPos.right, minWidth: 120, padding: '4px 0', zIndex: 99999 }}>
          <button className="btn btn-sm" style={{ width: '100%', textAlign: 'left', borderRadius: 0, border: 'none', justifyContent: 'flex-start', padding: '8px 14px', fontSize: 14, color: 'var(--danger)', gap: 8 }}
            onMouseDown={e => e.preventDefault()}
            onClick={() => {
      ...
      {confirmFile && createPortal(
        <div style={{ position: 'fixed', inset: 0, zIndex: 20000, background: 'rgba(0,0,0,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}
          onMouseDown={e => { if (e.target === e.currentTarget && !deletingFile) setConfirmFile(null); }}>
          <div style={{ background: 'var(--panel)', border: '1px solid var(--border)', borderRadius: 12, width: '100%', maxWidth: 420, boxShadow: '0 20px 60px rgba(0,0,0,0.35)', overflow: 'hidden' }}>
            <div style={{ padding: '20px 24px 0' }}><p style={{ fontSize: 16, fontWeight: 600, color: 'var(--text)', margin: '0 0 16px' }}>Delete file</p></div>
            <div style={{ height: 1, background: 'var(--border)' }} />
            <div style={{ padding: 24 }}>
              <p style={{ fontSize: 14, color: 'var(--text)', marginBottom: 20, lineHeight: 1.5 }}>Delete <strong>"{confirmFile}"</strong>?</p>
              <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
                <button onClick={() => setConfirmFile(null)} disabled={deletingFile} style={{ padding: '8px 16px', borderRadius: 8, border: '1px solid var(--border)', background: 'var(--panel-soft)', color: 'var(--text)', fontSize: 14, cursor: 'pointer' }}>Cancel</button>
                <button onClick={handleDeleteFileConfirmed} disabled={deletingFile} style={{ padding: '8px 16px', borderRadius: 8, border: 'none', background: 'var(--danger)', color: '#fff', fontSize: 14, cursor: 'pointer', opacity: deletingFile ? 0.7 : 1 }}>{deletingFile ? 'Deleting…' : 'Delete'}</button>
              </div>
            </div>
          </div>
        </div>, document.body,
      ...
      {confirmProposal && createPortal(
        <div style={{ position: 'fixed', inset: 0, zIndex: 20000, background: 'rgba(0,0,0,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}
          ...
          <div style={{ background: 'var(--panel)', border: '1px solid var(--border)', borderRadius: 12, width: '100%', maxWidth: 420, boxShadow: '0 20px 60px rgba(0,0,0,0.35)', overflow: 'hidden' }}>
            <div style={{ padding: '20px 24px 0' }}><p style={{ fontSize: 16, fontWeight: 600, color: 'var(--text)', margin: '0 0 16px' }}>Delete proposal</p></div>
            <div style={{ height: 1, background: 'var(--border)' }} />
            <div style={{ padding: 24 }}>
              <p style={{ fontSize: 14, color: 'var(--text)', marginBottom: 20, lineHeight: 1.5 }}>Delete the proposal for <strong>"{confirmProposal.client || confirmProposal.fileName}"</strong>?</p>
              <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
                <button onClick={() => setConfirmProposal(null)} disabled={deletingProposal} style={{ padding: '8px 16px', borderRadius: 8, border: '1px solid var(--border)', background: 'var(--panel-soft)', color: 'var(--text)', fontSize: 14, cursor: 'pointer' }}>Cancel</button>
                <button onClick={handleDeleteProposalConfirmed} disabled={deletingProposal} style={{ padding: '8px 16px', borderRadius: 8, border: 'none', background: 'var(--danger)', color: '#fff', fontSize: 14, cursor: 'pointer', opacity: deletingProposal ? 0.7 : 1 }}>{deletingProposal ? 'Deleting…' : 'Delete'}</button>
              </div>
            </div>
          </div>
        </div>, document.body,
      ...
      {confirmMicrosite && createPortal(
        <div style={{ position: 'fixed', inset: 0, zIndex: 20000, background: 'rgba(0,0,0,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}
          ...
          <div style={{ background: 'var(--panel)', border: '1px solid var(--border)', borderRadius: 12, width: '100%', maxWidth: 420, boxShadow: '0 20px 60px rgba(0,0,0,0.35)', overflow: 'hidden' }}>
            <div style={{ padding: '20px 24px 0' }}><p style={{ fontSize: 16, fontWeight: 600, color: 'var(--text)', margin: '0 0 16px' }}>Delete microsite</p></div>
            <div style={{ height: 1, background: 'var(--border)' }} />
            <div style={{ padding: 24 }}>
              <p style={{ fontSize: 14, color: 'var(--text)', marginBottom: 20, lineHeight: 1.5 }}>Delete the microsite for <strong>"{confirmMicrosite.displayName}"</strong>?</p>
              <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
                <button onClick={() => setConfirmMicrosite(null)} disabled={deletingMicrosite} style={{ padding: '8px 16px', borderRadius: 8, border: '1px solid var(--border)', background: 'var(--panel-soft)', color: 'var(--text)', fontSize: 14, cursor: 'pointer' }}>Cancel</button>
                <button onClick={handleDeleteMicrositeConfirmed} disabled={deletingMicrosite} style={{ padding: '8px 16px', borderRadius: 8, border: 'none', background: 'var(--danger)', color: '#fff', fontSize: 14, cursor: 'pointer', opacity: deletingMicrosite ? 0.7 : 1 }}>{deletingMicrosite ? 'Deleting…' : 'Delete'}</button>
              </div>
            </div>
          </div>
        </div>, document.body,
