...
export function ColorPaletteEditor({ tokens, onClose }: Props) {
  ...
  function handleReset() {
    ...
    ctx!.replaceAst(newAst);
    setLocalTokens({
      accent: tokens.accent, bg: tokens.bg,
      surface: tokens.surface, text: tokens.text,
    });
  }

  const hasOverrides = EDITABLE_TOKENS.some(t => ctx.ast.customTokens?.[t.key] !== undefined);
  ...
  return (
    ...
    >
      <div style={{ padding: '12px 14px', borderBottom: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <p style={{ margin: 0, fontSize: 12, fontWeight: 700, color: '#1e293b' }}>Color Palette</p>
          <p style={{ margin: '2px 0 0', fontSize: 11, color: '#94a3b8' }}>Edit global design tokens</p>
        </div>
        {hasOverrides && (
          <button
            onClick={handleReset}
            style={{ fontSize: 10, color: '#ef4444', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600, padding: '3px 6px' }}
          >Reset</button>
        )}
      </div>
        ...
        {EDITABLE_TOKENS.map(token => (
          <div key={token.key}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
              <div>
                <span style={{ fontSize: 11, fontWeight: 600, color: '#1e293b' }}>{token.label}</span>
                <span style={{ fontSize: 10, color: '#94a3b8', marginLeft: 6 }}>{token.description}</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                <div style={{ position: 'relative', width: 24, height: 24 }}>
                  ...
                  <input
                    ...
                    onChange={e => handleChange(token.key as string, e.target.value)}
                    style={{
                ...
                {ACCENT_PRESETS.map(color => (
                  <button
                    ...
                    style={{
                      ...
                      background: color,
                      border: localTokens.accent === color ? `2px solid ${ACCENT}` : '1.5px solid rgba(0,0,0,0.12)',
                      cursor: 'pointer',
                      padding: 0,
                      flexShrink: 0,
                    }}
