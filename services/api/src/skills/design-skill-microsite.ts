...
export async function generateSectionHtml(
  ...
  const prompt = [
    ...
    `- Card grid columns MUST use minmax(0, 1fr) NOT plain 1fr. Example: grid-template-columns: repeat(3, minmax(0, 1fr)). Plain "1fr" does not shrink below content width and causes cards to overflow and overlap each other. Every grid child that holds text must also have min-width:0.`,
    `- CARD ALIGNMENT RULES (prevent overlap and cut-off cards):`,
    `  * Every card element MUST have box-sizing:border-box so padding and borders stay inside the cell.`,
    `  * NEVER give cards a fixed pixel width (width:300px etc.) — use width:100% inside the grid/flex cell.`,
    `  * Flex card rows MUST have flex-wrap:wrap so cards reflow instead of overflowing the container.`,
    `  * NEVER use position:absolute for card placement inside a grid or flex row — absolute cards are removed from flow and overlap siblings.`,
    `  * Card grids and flex rows MUST have width:100% on the container so they don't exceed the section width.`,
    `  * If cards have equal height, use align-items:stretch on the grid/flex container, NOT position:absolute tricks.`,
    `  * Gaps between cards MUST use the grid/flex gap property — NEVER use negative margins to pull cards together, which causes overlap.`,
    ``,
    ...
    `C. Diagonal clip-path background strip (dark angled panel). CRITICAL CONTAINMENT RULE: ALL text — headlines, body paragraphs, lists, labels — MUST be inside a wrapper element that carries the same dark background color (not just the clip-path element). The light-background area must contain ONLY decorative visuals (images, shapes, icons) — NEVER readable text. Body copy that extends beyond the dark strip into the light area produces invisible white-on-light text, which is forbidden. Use a dark-bg wrapper div (same color as the clip-path panel) that encloses every text node and is NOT clipped.`,
