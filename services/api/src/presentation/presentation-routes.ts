...
ONLY fixed constraints:
  ...
  • All inner divs, grids, and card containers MUST use width:100% — NEVER use max-width or margin:auto centering on any element inside a slide. The full padded width (1280px minus 144px side-padding = 1136px) is your canvas; no element may restrict it further with a max-width constraint
  • The data-ls-content wrapper div MUST always be width:100% with NO max-width — writing max-width on data-ls-content is a critical error that breaks full-width layout
  • NEVER use margin:0 auto anywhere inside a slide — centering a content column with margin auto is forbidden; use flex justify-content or grid instead
  • NEVER set a width property on the body element — leave body width at its default (browser controls it); writing body{width:1280px} or any fixed width on body breaks the full-viewport scaling engine
...
CONTENT DENSITY — one idea per slide, never overpack:
  ...
  • Card grids and content rows MUST span the full slide width using display:grid with width:100% — never constrain them with a fixed pixel width or max-width
