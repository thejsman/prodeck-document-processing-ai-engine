...
// ---------------------------------------------------------------------------
// Template YAML update helper
// ---------------------------------------------------------------------------

/**
 * Update the instruction for a named section in the namespace's custom template YAML.
 * The template is at workdir/data/templates/{nsSlug}.yaml.
 * If the file doesn't exist or the section isn't found, the update is silently skipped.
 *
 * The YAML format generated by ensureTemplateYaml uses >- block scalars with a single
 * content line for instructions, so we can do a line-based replacement.
 */
async function updateTemplateSection(
  ...
  const nsSlug = namespace.toLowerCase().replace(/[^a-z0-9]+/g, '-');
  const templatePath = path.join(workdir, 'data', 'templates', `${nsSlug}.yaml`);

  try {
    await access(templatePath);
  } catch {
    return; // No namespace template — nothing to update
  }
  ...
  for (const line of lines) {
    if (skipNextLine) {
      // Replace instruction content line with sanitised new instruction
      result.push(`      ${newInstruction.replace(/\n/g, ' ').trim()}`);
      skipNextLine = false;
      continue;
    }
    ...
    if (titleMatch) {
      inTargetSection = titleMatch[1].trim() === sectionTitle;
    }

    if (inTargetSection && /^\s+instruction:\s*>-\s*$/.test(line)) {
      result.push(line);
      skipNextLine = true;
      continue;
    }
  ...
  await writeFile(templatePath, result.join('\n'), 'utf-8');
}
...
async function handleEditSection(
  ...
  // Update the namespace template YAML so future proposals reflect this edit preference
  void updateTemplateSection(workdir, namespace, target.heading, incomingMessage);
